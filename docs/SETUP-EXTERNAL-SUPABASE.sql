-- ============================================================================
-- LIORA — pełny schemat dla ZEWNĘTRZNEGO projektu Supabase
-- Cel: htyvrbeyzcpoktltdzsx.supabase.co
--
-- Uruchom CAŁY plik w SQL Editor swojego projektu (Supabase Studio).
-- Skrypt jest IDEMPOTENTNY (CREATE ... IF NOT EXISTS, guard na typie enum),
-- więc bezpiecznie można go uruchomić ponownie.
--
-- Zawiera: 12 tabel, enum app_role, funkcje has_role / booked_dates /
-- active_schedule_windows, polityki RLS oraz wymagane uprawnienia GRANT.
-- ============================================================================

BEGIN;


-- ─── źródło: supabase/migrations/20260809044150_d04b6d58-e3e2-4744-8517-3965510b849e.sql ───
...
-- ─── źródło: supabase/migrations/20260809111101_c6127cdc-4872-4e09-a368-1b4a19dcef61.sql ───
...
-- ─── źródło: supabase/migrations/20260809205203_56720a11-b60f-4284-815e-40af6a0b4acb.sql ───

REVOKE ALL ON public.analytics_events FROM anon;
GRANT SELECT ON public.analytics_events TO authenticated;
GRANT ALL ON public.analytics_events TO service_role;

-- ─── źródło: supabase/migrations/20260809042144_56cd4510-ed0b-4d3e-a7d7-8fcdd00fcd5b.sql ───
...
-- ============================================================================
-- KROK KOŃCOWY — nadanie sobie roli administratora.
-- Wykonaj PO pierwszym zalogowaniu się w aplikacji (link e-mail),
-- podstawiając swój adres:
--
--   INSERT INTO public.user_roles (user_id, role)
--   SELECT id, 'admin'::public.app_role FROM auth.users WHERE email = 'TWOJ@EMAIL'
--   ON CONFLICT (user_id, role) DO NOTHING;
--
-- Weryfikacja:
